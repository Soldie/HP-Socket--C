	...
	: CDialogEx(CServerDlg::IDD, pParent)
{
	...
	// 初始化 SSL 全局环境参数
	::HP_SSL_Initialize(SSL_SM_SERVER, g_s_iVerifyMode, g_s_lpszPemCertFile, g_s_lpszPemKeyFile, g_s_lpszKeyPasswod, g_s_lpszCAPemCertFileOrPath, nullptr);
...
{
	// 销毁 Socket 对象
	::Destroy_HP_SSLPullServer(m_pServer);
	// 销毁监听器对象
...
void CServerDlg::OnBnClickedStart()
{
	...
	if(::HP_Server_Start(m_pServer, ADDRESS, PORT))
	{
	...
	{
		::LogServerStartFail(::HP_Server_GetLastError(m_pServer), ::HP_Server_GetLastErrorDesc(m_pServer));
		SetAppState(ST_STOPPED);
	}
}
...
void CServerDlg::OnBnClickedDisconnect()
{
	...
	if(::HP_Server_Disconnect(m_pServer, dwConnID, TRUE))
