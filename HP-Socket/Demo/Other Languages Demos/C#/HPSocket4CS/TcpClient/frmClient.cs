    ...
    public partial class frmClient : Form
    {
        ...
        HandleResult OnReceive(IntPtr dwConnId, IntPtr pData, int iLength)
        {
            ...
            if (isSendFile == true)
            {
                ...
                AddMsg(string.Format(" > [{0},OnReceive] -> FileInfo(Path:\"{1}\",Size:{2})", dwConnId, myFile.FilePath, myFile.FileSize));
                AddMsg(string.Format(" > [{0},OnReceive] -> FileContent(\"{1}\")", dwConnId, msg));
            }
            else if (studentType != StudentType.None)
            {
                ...
                switch (studentType)
                {
                    case StudentType.Array:
                        ...
                        {
                            AddMsg(string.Format(" > [{0},OnReceive] -> Student({1},{2},{3})", dwConnId, stu.Id, stu.Name, stu.GetSexString()));
                        }
                        break;
                    ...
                    case StudentType.Single:
                        ...
                        AddMsg(string.Format(" > [{0},OnReceive] -> Student({1},{2},{3})", dwConnId, student.Id, student.Name, student.GetSexString()));
                        studentType = StudentType.None;
                        break;
                }
            }
