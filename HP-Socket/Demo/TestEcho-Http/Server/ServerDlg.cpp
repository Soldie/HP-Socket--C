...
BOOL CServerDlg::OnInitDialog()
{
	...
	strTitle.Format(_T("%s - (%s:%d/%d)"), strOriginTitle, ADDRESS, HTTP_PORT, HTTPS_PORT);
...
{
	if(::CheckIsIPAddress(lpszServerName))
		return 0;
...
void CServerDlg::OnBnClickedStart()
{
	...
	g_SSL.Cleanup();
	if(g_SSL.Initialize(SSL_SM_SERVER, SSL_VM_NONE, g_s_lpszPemCertFile, g_s_lpszPemKeyFile, g_s_lpszKeyPasswod, g_s_lpszCAPemCertFileOrPath, SIN_ServerNameCallback))
		SPECIAL_SERVER_INDEX = g_SSL.AddServerContext(SSL_VM_NONE, g_s_lpszPemCertFile2, g_s_lpszPemKeyFile2, g_s_lpszKeyPasswod2, g_s_lpszCAPemCertFileOrPath2);
	else
	{
	...
	if(m_HttpServer.Start(ADDRESS, HTTP_PORT))
	{
		::LogServerStart(ADDRESS, HTTP_PORT, HTTP_NAME);

		if(m_HttpsServer.Start(ADDRESS, HTTPS_PORT))
		{
