...
void CClientDlg::OnBnClickedStart()
{
	...
	if(m_Agent.Start(LOCAL_ADDRESS, FALSE))
	{
		for(int i = 0; i < m_iSocketCount; i++)
		{
			...
			{
				::LogClientStartFail(::GetLastError(), ::GetSocketErrorDesc(SE_CONNECT_SERVER));
				break;
			}
		}
	}
...
{
	//VERIFY(::SSO_SendBuffSize(socket, 512 * 1024) == NO_ERROR);
	//VERIFY(::SSO_RecvBuffSize(socket, 512 * 1024) == NO_ERROR);
