...
namespace SSLPackServerNS
{
    ...
    public partial class frmServer : Form
    {
        ...
        // 两种构造方式,第一种
        //HPSocketCS.SSLPackServer server = null;
        // 两种构造方式,第二种
        HPSocketCS.SSLPackServer server = new HPSocketCS.SSLPackServer(SSLVerifyMode.Peer | SSLVerifyMode.FailIfNoPeerCert, "ssl-cert\\server.cer", "ssl-cert\\server.key", "123456", "ssl-cert\\ca.crt", null);

        HPSocketCS.Extra<ClientInfo> extra = new HPSocketCS.Extra<ClientInfo>();

        private string title = "Echo-SSLPackServer [ 'C' - clear list box ]";
        public frmServer()
        {
            InitializeComponent();
        }
